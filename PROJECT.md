...
左侧收藏夹菜单来自 `collection.json`，支持“所有”和单个收藏夹筛选；搜索和回答/文章类型筛选继续叠加生效。

搜索使用一个输入框和“标题 / 作者”选择器，默认选择标题。导航卡片分别通过 `data-search-title` 和 `data-search-author` 提供显示标题和作者名；标题对回答使用问题标题，对文章使用文章标题。筛选仅匹配选定字段，不读取卡片全文、摘要或展开后的正文和评论。查询去除首尾空白并进行不区分大小写的部分匹配，空查询不限制结果。编辑中的范围和查询与已应用条件分开保存。切换范围只更新占位文字及可访问名称，保留查询内容；输入和清空均不触发搜索。点击 type="button" 的“搜索”按钮或在输入框按 Enter 会更新已应用条件并回到第一页，排序保持不变；输入法组合输入期间的 Enter 不提交。键盘提交复用按钮的点击处理。收藏夹、类型、排序及分页始终使用已应用条件，避免其它操作意外提交输入中的查询。窄屏下选择器与输入框保持同组，其它工具栏控件换行。
