...
搜索使用一个输入框和“标题 / 作者”选择器，默认选择标题。导航卡片分别通过 `data-search-title` 和 `data-search-author` 提供显示标题和作者名；标题对回答使用问题标题，对文章使用文章标题。筛选仅匹配选定字段，不读取卡片全文、摘要或展开后的正文和评论。查询去除首尾空白并进行不区分大小写的部分匹配，空查询不限制结果。切换范围时保留查询内容，更新占位文字及可访问名称，立即重新筛选并回到第一页，排序保持不变。窄屏下选择器与输入框保持同组，其它工具栏控件换行。
