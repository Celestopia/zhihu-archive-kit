...
assert.doesNotMatch(content, /更改保存目录/);
...
assert.match(content, /saveZipToCollection/);
assert.match(content, /findSavedCollectionsForFolder/);
assert.match(content, /已保存于/);
assert.match(content, /data-zhmd-folder-name/);
assert.doesNotMatch(content, /showDirectoryPicker/);
assert.doesNotMatch(content, /IndexedDB|indexedDB/);
assert.match(content, /本地归档服务未运行/);
