...
网页端保存到收藏夹需要先运行本地浏览服务。该服务负责把油猴脚本生成的内容写入应用数据目录。
...
先在项目目录运行 `start-local-browser.ps1`，或执行：
...
保存根目录下以下划线开头的一级目录是项目内部目录，不会作为收藏夹展示，也不能作为新收藏夹名称。批量保存目录 `_batch/` 使用这个规则。
...
- `output_dir`：可选的批量输出目录。省略时使用 `%LOCALAPPDATA%/Zhihu Archive Kit/data/_batch/`；相对路径以 JSON 配置文件所在目录为基准。
...
`preview.html` 与 `assets/` 位于同一文件夹内，可以直接用浏览器打开。页面上方显示回答或文章正文，下方是可展开的评论区。回答预览页还会显示所属问题的创建时间、修改时间、回答数、评论数、关注数、标签和问题描述。已知知乎表情会从 `[赞]` 这类文本渲染为图片；下载的表情资源缓存在 `%LOCALAPPDATA%/Zhihu Archive Kit/cache/emoji/`，渲染时以 data URI 写入 HTML，因此预览文件不依赖缓存目录。下载失败时保留原文本。每次运行会覆盖同名 `preview.html`，不会修改 `index.md`、`comments.json` 或 `assets/`。
...
真实知乎页面的 DOM、登录状态、油猴脚本与本地服务通信以及媒体下载行为需要在浏览器中手动验证。
